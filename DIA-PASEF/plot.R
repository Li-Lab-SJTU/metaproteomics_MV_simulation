...
result = read.csv('./simu_metrics/simu_metrics_24_10000_0.3_simulation1.txt',sep='\t')
...
ggsave("./plot/mouse_24_0.3_pAUROC.png",
       p1,
       device="png",
       dpi=1200,
       height=6,
...
ggsave("./plot/mouse_24_0.3_FPR.png",
       p2,
       device="png",
       dpi=1200,
       height=6,
...
mean(result_median_diff$diff_0.2_0.8_ratio[result_median_diff$diff_0.2_0.8_ratio < -0.15])

result_median_crossMNAR = aggregate(result$pAUROC, by=list(result$Nsample,result$fc,result$zr,result$model, result$Model_Type), FUN=function(x) median(x,na.rm=T))
result_mean_crossMNAR = aggregate(result$pAUROC, by=list(result$Nsample,result$fc,result$zr,result$model, result$Model_Type), FUN=function(x) mean(x,na.rm=T))
