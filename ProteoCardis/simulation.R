...
if (!paste0("round_", i, ".txt") %in% simu_oks){
  ...
  ## load data
  protein_data<-read.csv(file = "../datasets/ProteoCardis/Data-sets/Proteocardis-cyto-proteins-XIC-intensities.csv",header = TRUE,sep = ",")
  protein_data_need<-data.frame(protein_data)
  rm(protein_data)
  rownames(protein_data_need)<-protein_data_need[,'Feature']
  protein_data_need<-protein_data_need[,2:100]
  ...
  # random seeds
  set.seed(round(samplesize*featuresize*fc*(exp(mr_all)**exp(mr_nrandom))+7*i))
  ...
  print(paste0(simu_result_dir, "/round_", i, ".txt"))
  ...
  print(paste("Total elapsed", all_elapsed[["elapsed"]] / 60, "min."))
  print(paste("Simulation_", samplesize,
                                          "_", featuresize,
                                          "_", fc, 
                                          "_", mr_all, 
