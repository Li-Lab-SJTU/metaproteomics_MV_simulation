...
## 开始进行数据集分析
protein_data<-read.csv(file = "../datasets/ALTR/AL-vs-TR_Proteins.txt",header = TRUE,sep = "\t")
protein_data_need<-data.frame(protein_data)
protein_data_need = protein_data_need %>% filter(Contaminant == 'False')
protein_data_need = protein_data_need[,c(5,51:66)]
rownames(protein_data_need) = protein_data_need$Accession
protein_data_need = protein_data_need[-1]
meta = read.csv("../datasets/ALTR/AL-vs-TR_InputFiles.txt",header = TRUE,sep = "\t")

colnames(protein_data_need) = sapply(str_split(colnames(protein_data_need),'Normalized...'),function(x) x[[2]])
colnames(protein_data_need) = sapply(str_split(colnames(protein_data_need),'..Sample'),function(x) x[[1]])
colnames(protein_data_need) = meta$File.Name[sapply(meta$File.ID,function(x) which(x == colnames(protein_data_need)))]
protein_data_need_group1 = str_detect(colnames(protein_data_need),'AL_')
...
# filter NA>90%
protein_data_need<-protein_data_need[which(apply(protein_data_need,1,function(x) sum(is.na(x)))<=0.9*ncol(protein_data_need)),]
meansd_data<-log(protein_data_need+1)
